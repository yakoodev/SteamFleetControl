...
- QR-онбординг на странице создания аккаунта: скан через Steam Guard и автосоздание карточки.
- Импорт/экспорт аккаунтов, теги, папки, автоматическая синхронизация Steam Family (включая внешних участников).
...
QR-онбординг:
1. Откройте `Аккаунты -> Создать`.
2. Нажмите `Добавить через QR`.
3. Сканируйте QR в мобильном Steam Guard.
4. После подтверждения сервис автоматически создаст аккаунт и откроет карточку.

Опциональный bootstrap SuperAdmin:
- Установите `ADMIN_EMAIL` и `ADMIN_PASSWORD`, если хотите создать/повысить SuperAdmin при старте.
- Если переменные не заданы, bootstrap админа пропускается.
